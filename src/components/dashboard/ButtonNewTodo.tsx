...
const validationTodoFormSchema = z.object({
  ...
  price: z.string().min(1, { message: "Informe um preço" }),
  representante_id: z.number().min(1, { message: "Informe um representante" } )
});
  ...
  return(
      <Dialog.Root>
        <Dialog.Trigger asChild>
          <button type='submit' className='w-full h-fit p-4 bg-black text-white rounded-md flex justify-center items-center gap-4'>
        ...
        <Dialog.Portal className='w-full h-screen'>
          ...
          <Dialog.Content className="w-[85%] md:w-[65%] lg:w-[30%] h-fit p-8 rounded-md top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white fixed z-[100] focus-visible:outline-0 transition-all">
              <div className='w-full flex justify-between items-center mb-8'>
                <h1 className='text-2xl text-center flex-1 font-sans font-bold text-black'>Criar To-do</h1>
            ...
            <form onSubmit={handleSubmit(handleTodo)} className='flex flex-col gap-4'>
              <div className="w-full flex flex-col gap-2">
                <input {...register('product_id', { valueAsNumber: true })}  type='number' placeholder='Id do produto' className='w-full px-2 py-2 text-black text-sm font-normal rounded-lg border-2 border-grayBorder focus:outline-black'  />
              ...
              <select {...register('representante_id', { valueAsNumber: true })} className='w-full bg-grayBack text-black text-sm px-4 py-2 rounded-lg'>
